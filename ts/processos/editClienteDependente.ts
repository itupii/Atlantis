...
import MenuEditarClienteDependente from "../menus/menuEditClienteDP";
...
export default class EditarClienteDependente extends Processo {
    ...
    processar(): void {
        console.clear()
        var processo = false
        let dependente = this.entrada.recebeTexto('| Insira o número do documento do cliente dependente a ser editado:')
            ...
            for (let indexDep = 0; indexDep < this.clientes[index].Dependentes.length; indexDep++) {
                for (let indexDoc = 0; indexDoc < this.clientes[index].Dependentes[indexDep].Documentos.length; indexDoc++) {
                    if (dependente == this.clientes[index].Dependentes[indexDep].Documentos[indexDoc].Numero) {
                        ...
                        switch (this.opcao) {
                            case 1:
                                let novoNomeDep = this.entrada.recebeTexto('| Insira o novo nome do cliente: ')
                                this.clientes[index].Dependentes[indexDep].Nome = novoNomeDep;
                                break;
                            case 2:
                                let novoNomeSocialDep = this.entrada.recebeTexto('| Insira o novo nome social do cliente: ')
